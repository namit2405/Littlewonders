...
// Lazy init — prevents crash on startup if env var not yet loaded
let _resend = null;
function getResend() {
  if (!_resend) {
...
async function sendEnrolmentEmail(data) {
  ...
  if (error) { console.error('[Mailer] Enrolment email FAILED:', error); throw new Error(error.message); }
...
async function sendContactEmail(data) {
  ...
  if (error) { console.error('[Mailer] Contact email FAILED:', error); throw new Error(error.message); }
